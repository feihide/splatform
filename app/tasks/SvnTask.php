 ...
 * Time: 下午7:30
 */
class SvnTask extends \Phalcon\CLI\Task
{
...
class svn
{
    ...
    public function __construct()
    {
        ...
        $this->repos['api_trunk']['path']     = 'http://192.168.1.143/svn/api/trunk';
        $this->repos['api_trunk']['username'] = 'zhangyifei';
        $this->repos['api_trunk']['password'] = 'feihide';

        $this->repos['api_branches']['path']     = 'http://192.168.1.143/svn/api/branches';
        $this->repos['api_branches']['username'] = 'zhangyifei';
        $this->repos['api_branches']['password'] = 'feihide';

        $this->repos['jrd_trunk']['path']     = 'http://192.168.1.143/svn/jrd/trunk';
        $this->repos['jrd_trunk']['username'] = 'zhangyifei';
        $this->repos['jrd_trunk']['password'] = 'feihide';

        $this->repos['jrd_branches']['path']     = 'http://192.168.1.143/svn/jrd/branches';
        $this->repos['jrd_branches']['username'] = 'zhangyifei';
        $this->repos['jrd_branches']['password'] = 'feihide';

        $this->encodings = 'utf-8, gbk';
        $this->client    = 'svn';
    }
    ...
    public function run()
    {
        ...
        foreach($this->repos as $name => $repo)
        {
            ...
            foreach($logs as $log)
            {
                ...
                $data        = new SvnLog();
                $data->author = $log->author;
                $data->revision =  $log->revision;
                $data->files = $log->files;
                $data->msg  =$log->msg;
                $data->repo = $name;
                $data->date = strtotime($log->date);
                if ($data->save() == false) {
                    echo "日志插入失败 \n";
...
//                }
    ...
    {
        $this->logRoot =APPLICATION_PATH . '/svn_log/';
    ...
    {
        if(empty($this->repos)) die("You must set one svn repo.\n");
    }
    ...
    {
        if($this->client == '') die("You must set the svn client file.\n");
        $this->client = $this->client . " --non-interactive";
        if(stripos($repo->path, 'https') === 0 or stripos($repo->path, 'svn') === 0)
        {
            $cmd = $this->client . ' --version --quiet';
            $version = `$cmd`;
